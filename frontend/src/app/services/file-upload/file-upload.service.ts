...
export class FileQueueObject {
  ...
  public request: Subscription | any = null;
  public response: HttpResponse<any> | any | HttpErrorResponse = null;
...
export class FileUploadService {
  ...
  // public functions
  public addToQueue(data: any, localeId: string) {
    // add file to the queue
    _.each(data, (file: any) => this._addToQueue(file, localeId));
  }
  ...
  public uploadAll(localeId: string) {
    // upload all except already successfull or in progress
    _.each(this._files, (queueObj: FileQueueObject) => {
      if (queueObj.isUploadable()) {
        this._upload(queueObj, localeId);
      }
    });
  }

  // private functions
  private _addToQueue(file: any, localeId: string) {
    const queueObj = new FileQueueObject(file);

    // set the individual object events
    queueObj.upload = () => this._upload(queueObj, localeId);
  ...
  private _upload(queueObj: FileQueueObject, localeId: string) {
    // create form data for file
    const form = new FormData();
    form.append('file', queueObj.file, queueObj.file.name);
    console.log(form)
    // upload file and report progress
    const req = new HttpRequest('POST', environment.backendURL + 'locales/' + localeId + '/images', form, {
      reportProgress: true,
    });
